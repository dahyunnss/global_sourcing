...
# API 호출 및 데이터 처리 함수들
# ================================
def build_url(query: str, page: int, page_size: int, extras: Dict[str, str] = None) -> str:
    q = query
    common = {
        "query": q, #검색 키워드
        "page": page, #페이지 번호(1부터 시작)
        "pageSize": page_size, #페이지당 결과수
        "langident": "zh",
        "country": "CN", #국가 필터
        "tab":"supplier",
        # 캐시/세션 회피용
        "startTime": now_ms(),  # 현재 타임스탬프 (밀리초)
        "requestId": f"AI_Web_{hashlib.md5(str(time.time()).encode()).hexdigest()[:24]}_{now_ms()}",  # 고유 요청 ID (캐시 회피용)
        "queryRaw": q,
        "queryMachineTranslate": q,
        "queryProduct": q,
        "productName": q,
        "supplierQpKeywords": q,
        "supplierQpProductName": q,
        "productQpKeywords": q,
        "intention": "P",
        "supplierAttributes": "unknown",
        "productAttributes": "",
    }
    if extras:
...
def collect_single_keyword(query: str, out_root: str, page_size: int = 20, max_pages: int = 20, cookie: str = None) -> bool:
    ...
    for page in range(1, max_pages + 1):
        ...
        if len(cards) > 0:
            has_data = True  # 최소 1개 이상의 데이터가 있음
...
def merge_all_keywords_json(base_dir: Path, output_filename: str = "all_keywords_suppliers_en.json"):
...
def convert_json_to_csv(
    ...
    if output_csv_path is None:
        output_csv_path = json_path.parent / "all_keywords_suppliers_en.csv"
...
def main(csv_path: str = "", auto_confirm: bool = False, output_root: str = None):
    ...
    if output_root:
        ...
        OUTPUT_ROOT = os.path.join(_base_dir, "sourcing_data", f"supplier_search_{date_tag}_en")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        candidates = sorted(glob.glob(os.path.join(script_dir, "en_keywords_*.csv")),
                            key=os.path.getmtime, reverse=True)
        if not candidates:
            sys.__stdout__.write("❌ en_keywords_*.csv 파일을 찾을 수 없습니다.\n")
            sys.exit(1)
